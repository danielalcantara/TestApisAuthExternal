...
public class PagamentoReportBO implements IPagamentoReportBO {
	...
	public boolean processarPedidos() {
		...
		try {
			...
			List<LancamentoPedidoModel> listLancamentoPedido = null;
			String idsContratos = PropertyUtil.getConfigParamByKey("ids.contracts.process");

			if (idsContratos != null && !idsContratos.trim().isEmpty())
				listLancamentoPedido = agruparLancamentosPorPedido(
						pagamentoReportDAO.fetchConsumosPorContrato(idsContratos));
			else
	...
	public boolean fecharReportsVencidos() {
		...
		String fecharReportsVencidos = PropertyUtil.getConfigParamByKey("close.losers.reports");

		if (fecharReportsVencidos != null && fecharReportsVencidos.trim().equals("false")) {
			logger.info(
					"======= SISTEMA EM AMBIENTE DE TESTES OU HOMOLOGAÇÃO, FECHAMENTO DE REPORTS VENCIDOS DESATIVADO ========");
			return true;
		}
		
		logger.info("======= VERIFICANDO SE HÁ REPORTS PARA SEREM FECHADOS ========");

		String msgFechamentoResult = "======= NÃO HOUVE REPORTS HÁ SEREM FECHADOS! ========";
		boolean successProcess = true;
		...
		try {
			...
			if (lancamentoPeriodoModels != null && !lancamentoPeriodoModels.isEmpty()) {
				...
				if (countFechamentos > 0)
					msgFechamentoResult = "======= QUANTIDADE DE REPORTS FECHADOS: " + countFechamentos + " ========";
			...
			logger.info(msgFechamentoResult);
