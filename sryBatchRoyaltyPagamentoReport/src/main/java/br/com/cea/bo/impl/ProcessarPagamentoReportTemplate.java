...
public abstract class ProcessarPagamentoReportTemplate implements IProcessarPagamentoReportBO {
	...
	public boolean fechamentoReport(LancamentoPeriodoModel lancamentoPeriodo, PeriodoModel periodoModel)
		...
		if (DateUtil.compareDateWithoutTime(lancamentoPeriodo.getDataFimReport(), DateUtil.getCurrentDate()) <= 0) {
			...
			if (nextReport != null) {
				...
				if (FLAG_REPORT_FECHADO.equals(lancamentoPeriodo.getStatusReport())) {
					nextReport.setValorSaldo(nextReport.getValorSaldo() - lancamentoPeriodo.getValorConsumoDiario());
					nextReport.setValorSaldoParcelaPendente(
							nextReport.getValorSaldo() - lancamentoPeriodo.getValorConsumoDiario());
				} else {
				...
				processarPagamentoConsumo(nextReport, true);
	...
	protected void processarSaldo(LancamentoPeriodoModel lancamentoPeriodo, List<ParcelaModel> parcelas)
		...
		processarPagamentoConsumo(lancamentoPeriodo, false);
	...
	private boolean ehUltimoSaldoNegativo(Long idContrato) throws DataAccessException, SQLException{
		PagamentoModel pagamento = pagamentoReportDAO.getPagamentoUltimoSaldo(idContrato);
		if (pagamento != null && pagamento.getValorSaldo() < 0D){
			return true;
		}
		return false;
	}
	...
	protected boolean processarPagamentoConsumo(LancamentoPeriodoModel lancamentoPeriodoModel, boolean fechamento)
		...
		if (lancamentoPeriodoModel.getValorSaldo() < 0D)
			if (ehUltimoSaldoNegativo(lancamentoPeriodoModel.getIdContrato())){
				pagamento.setValorPagamento(lancamentoPeriodoModel.getValorConsumoDiario());
			} else {	
	...
	protected PagamentoModel geraPagamentoComplementar(LancamentoPeriodoModel lancamentoPeriodoModel, Double valorPagamento){
		PagamentoModel pagamento = new PagamentoModel();
		
		pagamento.setNumeroContrato(lancamentoPeriodoModel.getIdContrato());
		pagamento.setDataPagamento(lancamentoPeriodoModel.getDataFimReport());
		pagamento.setValorConsumo(0D);
		pagamento.setValorPagamento(valorPagamento);
		pagamento.setValorParcela(valorPagamento);
		pagamento.setValorSaldo(lancamentoPeriodoModel.getValorSaldo() + (valorPagamento) * -1);
		pagamento.setValorSaldoParcelaPendente(lancamentoPeriodoModel.getValorSaldo() + (valorPagamento) * -1);
		pagamento.setIdStatus(STATUS_PAGAMENTO_PENDENTE);
		return pagamento;
	}
